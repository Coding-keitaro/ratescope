...
export default function CapRateSimulator() {
  const [interestRate, setInterestRate] = useState(1.5);
  const [annualNOI, setAnnualNOI] = useState(1000);
  ...
  const priceChangeStatus = priceChange >= 0 ? 'positive' : 'negative';
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm border border-slate-100 dark:border-slate-700">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
            ...
            <ReferenceDot
              x={interestRate}
              y={Math.round(currentValueOku * 10) / 10}
              r={6}
              fill="#ef4444"
              stroke="#fff"
              strokeWidth={2}
