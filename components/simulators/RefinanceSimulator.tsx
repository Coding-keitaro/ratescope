  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <MetricCard
          label="当初 年間金利負担"
          value={`${beforeInterest.toLocaleString()}万円`}
          note={`借入 ${loanAmount}億 × 金利 ${initialRate.toFixed(1)}%`}
        />
        <MetricCard
          label="借換後 年間金利負担"
          value={`${afterInterest.toLocaleString()}万円`}
          note={`借入 ${loanAmount}億 × 金利 ${refinanceRate.toFixed(1)}%`}
        />
        <MetricCard
          label="コスト増加額"
          value={`+${costIncrease.toLocaleString()}万円`}
          note="借換後の年間増加分"
          status="negative"
        />
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm border border-slate-100 dark:border-slate-700">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={timelineData} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
            ...
            <ReferenceLine
              ...
              stroke="#94a3b8"
              strokeDasharray="4 4"
              label={{ value: '借換', position: 'top', fontSize: 11, fill: '#94a3b8' }}
            />
            <Bar dataKey="interest" radius={[4, 4, 0, 0]} isAnimationActive={false}>
              {timelineData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.isPost ? '#ef4444' : '#3b82f6'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
        <div className="flex gap-4 justify-center mt-2 text-xs text-slate-400 dark:text-slate-500">
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 rounded bg-blue-500 inline-block" /> 1〜5年目
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 rounded bg-red-500 inline-block" /> 6〜8年目（借換後）
          </span>
        </div>
      </div>
    </div>
